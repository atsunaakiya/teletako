...
def monitor_key(type_: MessageType, tag: str) -> str:
    return f"{MONITOR_PREFIX}:{type_.value}:{tag}"


def relation_key(type_: MessageType, tag: str) -> str:
    return f"{RELATION_PREFIX}:{type_.value}:{tag}"


def relation_id_key(type_: MessageType, tag: str) -> str:
    return f"{RELATION_ID_PREFIX}:{type_.value}:{tag}"
